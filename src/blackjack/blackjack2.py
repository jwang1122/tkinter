...
class MainFrame(BaseWindow):
    ...
    def setSeat(self, name, seat):
        self.boardFrame.setSeat(name, seat)
...
class ConfigFrame():
    def __init__(self, parent):
        ...
        playerBtn = Button(buttonFrm, text="Config Player", command=self.setPlayer)
        playerBtn.grid(row=0, column=0)
        bgBtn = Button(buttonFrm, text="Config Background", command=self.setBackground)
    ...
    def setPlayer(self):
        self.playPopup = Toplevel(self.frame)
        rootX = self.frame.winfo_rootx()+350
        rootY = self.frame.winfo_rooty()+200
        self.playPopup.geometry(f'250x150+{rootX}+{rootY}')
        self.playPopup.title("set player's seat")
        self.playPopup.configure(bg='yellow')
        Label(self.playPopup, text="Player name:",bg='yellow').grid(row=0,column=0, pady=10)
        self.playerNameEnt = Entry(self.playPopup, width=23)
        self.playerNameEnt.grid(row=0, column=1)
        self.playerNameVar = StringVar()
        self.playerNameVar.set("EAST")
        Label(self.playPopup, text="Available Seats:",bg='yellow').grid(row=1, column=0)
        self.combobox = Combobox(self.playPopup,textvariable=self.playerNameVar,values=["EAST","SOUTH","WEST"])
        self.combobox.grid(row=1, column=1)
        setBtn = Button(self.playPopup, text="Set", width=20, command=self.setSeat)
        setBtn.place(x=60, y=120)

    def setSeat(self):
        name = self.playerNameEnt.get()
        seat = self.combobox.get()
        self.parent.setSeat(name, seat)
...
class PlayBoardFrame():
    def __init__(self, parent):
        ...
        self.playerNameLbls = dict()
        self.displayPlayerNames()
        self.buildResultFrame(self.frame)
        
    def setSeat(self, playerName, seat):
    ...
    def updatePlayerName(self):
        for player in self.playerList:
            lbl = self.playerNameLbls[player.seat]
    ...
    def displayPlayerNames(self):
        for player in self.playerList:
            ...
            self.playerNameLbls[player.seat] = lbl
